...
find_files() {
  # $1 = route name
  grep -RIl --include="*.php" "->name('$1')" routes app 2>/dev/null || true
}
...
canonical_for() {
  ...
  if [ "$n" = "vol.dashboard" ] && grep -q "->name('vol.dashboard')" routes/me.php 2>/dev/null; then
...
if [ "${APPLY:-0}" = "1" ]; then
  ...
  echo "[DRY-RUN] To apply changes set APPLY=1, e.g.: APPLY=1 sh scripts/dedupe_route_names_v2.sh"
fi
