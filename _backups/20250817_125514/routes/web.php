...
/* Social auth placeholders (temporary) */
if (!Route::has('uaepass.redirect')) {
    Route::get('/auth/uaepass', fn() => view('auth.oauth-coming-soon', ['provider' => 'UAEPass']))->name('uaepass.redirect');
}
if (!Route::has('oauth.google.redirect')) {
    Route::get('/auth/google', fn() => view('auth.oauth-coming-soon', ['provider' => 'Google']))->name('oauth.google.redirect');
}
